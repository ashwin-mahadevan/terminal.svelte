...
export class AccessibilityManager {
	...
	constructor(
		...
		this._bottomBoundaryFocusListener = (e) =>
			this._handleBoundaryFocus(e, BoundaryPosition.BOTTOM);
	...
	private _handleBoundaryFocus(e: FocusEvent, position: BoundaryPosition): void {
		...
		const beforeBoundaryElement =
			this._rowElements[position === BoundaryPosition.TOP ? 1 : this._rowElements.length - 2];
	...
	private _handleSelectionChange(): void {
		...
		if (
			begin.node.compareDocumentPosition(end.node) & Node.DOCUMENT_POSITION_PRECEDING ||
			(begin.node === end.node && begin.offset > end.offset)
		) {
			[begin, end] = [end, begin];
		}
		...
		if (
			begin.node.compareDocumentPosition(this._rowElements[0]) &
		...
		if (
			end.node.compareDocumentPosition(lastRowElement) &
		...
		if (
			beginRowColumn.row > endRowColumn.row ||
			(beginRowColumn.row === endRowColumn.row && beginRowColumn.column >= endRowColumn.column)
		) {
