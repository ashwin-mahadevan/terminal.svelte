...
class TestTerminal extends CoreBrowserTerminal {
  public keyDown(ev: any): boolean | undefined { return (this as any)._keyDown(ev); }
  public keyPress(ev: any): boolean { return (this as any)._keyPress(ev); }
...
describe('CoreBrowserTerminal', () => {
  ...
  beforeEach(() => {
    ...
    (term as any)._compositionHelper = { get isComposing() { return false; }, keydown: () => true };
    (term as any).element = {
      classList: {
        toggle: () => { },
        remove: () => { }
  ...
  describe('events', () => {
    it('should fire the onData evnet', () => new Promise<void>(done => {
      term.onData(e => { expect(e).toBe('fake'); done(); });
      term.coreService.triggerDataEvent('fake');
    }));
    it('should fire the onCursorMove event', () => {
      return new Promise<void>(async r => {
        let fired = false;
        term.onCursorMove(() => { fired = true; expect(fired).toBe(true); r(); });
    ...
    it('should fire the onLineFeed event', () => {
      return new Promise<void>(async r => {
        let fired = false;
        term.onLineFeed(() => { fired = true; expect(fired).toBe(true); r(); });
        await term.writeP('\n');
      });
    });
    it('should fire a scroll event when scrollback is created', () => {
      return new Promise<void>(async r => {
        term.onScroll(e => { expect(typeof e).toBe('number'); r(); });
        await term.writeP('\n'.repeat(INIT_ROWS));
      });
    });
    ...
    it('should fire a key event after a keypress DOM event', () => new Promise<void>(done => {
      term.onKey(e => {
    ...
    it('should fire a key event after a keydown DOM event', () => new Promise<void>(done => {
      term.onKey(e => {
        expect(typeof e.key).toBe('string');
        expect(e.domEvent instanceof Object).toBe(true);
        done();
    ...
    it('should fire the onResize event', () => new Promise<void>(done => {
      term.onResize(e => {
    ...
    it('should fire the onScroll event', () => new Promise<void>(done => {
      term.onScroll(e => {
    ...
    it('should fire the onTitleChange event', () => new Promise<void>(done => {
    ...
    it('should fire the onBell event', () => new Promise<void>(done => {
      let fired = false;
      term.onBell(() => {
        fired = true;
  ...
  describe('paste', () => {
    ...
    it('should fire data event', () => new Promise<void>(done => {
      term.onData(e => {
        expect(e).toBe('foo');
        done();
      });
    ...
    describe('scroll() function', () => {
      describe('when scrollback > 0', () => {
        it('should create a new line and scroll', () => {
          ...
          expect(term.buffer.lines.get(INIT_ROWS - 1)!.loadCell(0, new CellData()).getChars()).toBe('b');
      ...
      describe('when scrollback === 0', () => {
        ...
        it('should create a new line and shift everything up', () => {
          ...
          expect(term.buffer.lines.get(INIT_ROWS - 2)!.loadCell(0, new CellData()).getChars()).toBe('c');
          expect(term.buffer.lines.get(INIT_ROWS - 1)!.loadCell(0, new CellData()).getChars()).toBe('');
        });
  ...
  describe('Third level shift', () => {
    ...
    describe('On Mac OS', () => {
      ...
      it('should emit key with alt + key on keyPress', () => new Promise<void>(done => {
        const keys = ['@', '@', '\\', '\\', '|', '|'];
    ...
    describe('On MS Windows', () => {
      ...
      it('should emit key with alt + ctrl + key on keyPress', () => new Promise<void>(done => {
        const keys = ['@', '@', '\\', '\\', '|', '|'];
  ...
  describe('unicode - surrogates', () => {
    for (let i = 0xDC00; i <= 0xDCF0; i += 0x10) {
      ...
      it(`${range}: 2 characters at last cell`, async () => {
        ...
        for (let idx = 0; idx < values.length; idx++) {
          const expected = values[idx];
          expect(term.buffer.lines.get(idx)!.loadCell(term.cols - 1, cell).getChars()).toBe(expected);
          expect(term.buffer.lines.get(idx)!.loadCell(term.cols - 1, cell).getChars().length).toBe(2);
          expect(term.buffer.lines.get(idx + 1)!.loadCell(0, cell).getChars()).toBe('');
        }
      });
      it(`${range}: 2 characters per cell over line end with autowrap`, async (): Promise<void> => {
        ...
        for (let idx = 0; idx < values.length; idx++) {
          const expected = values[idx];
          const row = idx * 2;
          expect(term.buffer.lines.get(row)!.loadCell(term.cols - 1, cell).getChars()).toBe('a');
          expect(term.buffer.lines.get(row + 1)!.loadCell(0, cell).getChars()).toBe(expected);
          expect(term.buffer.lines.get(row + 1)!.loadCell(0, cell).getChars().length).toBe(2);
          expect(term.buffer.lines.get(row + 1)!.loadCell(1, cell).getChars()).toBe('');
        }
      });
      it(`${range}: 2 characters per cell over line end without autowrap`, async (): Promise<void> => {
        ...
        for (let idx = 0; idx < values.length; idx++) {
          ...
          expect(term.buffer.lines.get(idx)!.loadCell(term.cols - 1, cell).getChars()).toBe(expected);
          expect(term.buffer.lines.get(idx)!.loadCell(term.cols - 1, cell).getChars().length).toBe(2);
          expect(term.buffer.lines.get(idx + 1)!.loadCell(1, cell).getChars()).toBe('');
        }
      });
