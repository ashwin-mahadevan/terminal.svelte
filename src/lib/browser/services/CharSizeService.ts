...
export class CharSizeService implements ICharSizeService {
	...
	private _measureStrategy: BaseMeasureStategy;
	...
	constructor(
		...
		this._fontChangeListener = this._optionsService.onMultipleOptionChange(
			['fontFamily', 'fontSize'],
			() => this.measure()
		);
	}
	...
	public dispose(): void {
		...
		this._measureStrategy.dispose();
		this._fontChangeListener.dispose();
	}
...
class DomMeasureStrategy extends BaseMeasureStategy {
	...
	public measure(): Readonly<IMeasureResult> {
		...
		this._validateAndSet(
			Number(this._measureElement.offsetWidth) / DomMeasureStrategyConstants.REPEAT,
...
class TextMetricsMeasureStrategy extends BaseMeasureStategy {
	...
	constructor(private _optionsService: IOptionsService) {
		super();
