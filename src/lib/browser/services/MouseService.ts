...
type RequestedMouseEvents = Record<
	'mouseup' | 'wheel' | 'mousedrag' | 'mousemove',
	EventListener | null
>;
...
export class MouseService {
	...
	public bindMouse(
		...
		register(
			this._mouseStateService.onProtocolChange((events) => {
				this._handleProtocolChange(ctx, eventListeners, events);
			})
		);
		register(
			this._optionsService.onSpecificOptionChange('mouseEventsRequireAlt', () => {
		 ...
		 */
		register(
			addDisposableListener(element, 'mousedown', (ev: MouseEvent) =>
				this._handleMouseDown(ctx, ev)
			)
		);
		register(
			addDisposableListener(
				element,
				'wheel',
				(ev: WheelEvent) => this._handlePassiveWheel(ctx, ev),
				{ passive: false }
			)
		);
		register(Gesture.addTarget(target.screenElement));
		register(
			addDisposableListener(target.screenElement, GestureEventType.START, () =>
				this._handleTouchStart()
			)
		);
		register(
			addDisposableListener(target.screenElement, GestureEventType.CHANGE, (e: IGestureEvent) =>
				this._handleTouchChange(ctx, e)
			)
		);
	}
	...
	private _sendEvent(ctx: IMouseBindContext, ev: MouseEvent | WheelEvent): boolean {
		...
		const pos = this._mouseCoordsService.getMouseReportCoords(
			ev as MouseEvent,
			ctx.target.screenElement
		);
		if (!pos) {
			return false;
		}
	...
	private _handleMouseDown(ctx: IMouseBindContext, ev: MouseEvent): void {
		...
		if (
			!this._mouseStateService.areMouseEventsActive ||
			this._selectionService.shouldForceSelection(ev)
		) {
			return;
		}
	...
	private _handleProtocolChange(
		ctx: IMouseBindContext,
	...
	private _triggerMouseEvent(e: ICoreMouseEvent): boolean {
		...
		if (
			...
			(e.action === CoreMouseAction.LEFT || e.action === CoreMouseAction.RIGHT)
		) {
			return false;
		}
...
export class AltMouseCursorController {
	...
	public sync(): void {
		...
		if (targetWindow) {
			store.add(
				addDisposableListener(targetWindow, 'blur', () => {
					if (this._isActive()) {
						this.resetClass();
					}
				})
