...
export class SelectionService {
	...
	protected _areCoordsInSelection(
	...
	private _getMouseEventScrollAmount(event: MouseEvent): number {
		...
		offset = Math.min(
			Math.max(offset, -Constants.DRAG_SCROLL_MAX_THRESHOLD),
			Constants.DRAG_SCROLL_MAX_THRESHOLD
		);
	...
	public shouldColumnSelect(event: KeyboardEvent | MouseEvent): boolean {
		...
		return (
			event.altKey &&
			!(Browser.isMac && this._optionsService.rawOptions.macOptionClickForcesSelection)
		);
	}
	...
	private _getWordAt(
		coords: [number, number],
		allowWhitespaceOnlySelection: boolean,
		followWrappedLinesAbove: boolean = true,
		...
		if (line.charAt(startIndex) === ' ') {
			...
			while (
				startCol > 0 &&
				startIndex > 0 &&
				!this._isCharWordSeparator(bufferLine.loadCell(startCol - 1, this._workCell))
			) {
			...
			while (
				...
				!this._isCharWordSeparator(bufferLine.loadCell(endCol + 1, this._workCell))
			) {
		...
		if (followWrappedLinesAbove) {
			if (start === 0 && bufferLine.getCodePoint(0) !== 32 /* ' ' */) {
				...
				if (
					...
					previousBufferLine.getCodePoint(this._bufferService.cols - 1) !== 32 /* ' ' */
				) {
		...
		if (followWrappedLinesBelow) {
			if (
				...
				bufferLine.getCodePoint(this._bufferService.cols - 1) !== 32 /* ' ' */
			) {
