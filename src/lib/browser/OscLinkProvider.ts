...
export class OscLinkProvider implements ILinkProvider {
	...
	public provideLinks(y: number, callback: (links: ILink[] | undefined) => void): void {
		...
		const result: ILink[] = [];
		const linkHandler = this._terminal.optionsService.rawOptions.linkHandler;
		...
		for (let x = 0; x < lineLength; x++) {
			...
			if (finishLink || (currentStart !== -1 && x === lineLength - 1)) {
				const text = this._terminal.oscLinkService.getLinkData(currentLinkId)?.uri;
				if (text) {
					...
					if (!ignoreLink) {
						...
						result.push({
							...
							range,
							activate: (e, text) =>
								linkHandler ? linkHandler.activate(e, text, range) : defaultActivate(e, text),
	...
	private _getRangeWithLineWrap(
		...
		while (finalStartX === 0) {
			...
			if (
				...
				!this._hasUrlId(previousLine, previousLineLength - 1, linkId)
			) {
				break;
			}
