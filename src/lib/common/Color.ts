		...
		describe('channels', () => {
			...
			describe('toColor', () => {
				it('should convert an rgb array to an IColor', () => {
					...
					expect(channels.toColor(0xa0, 0xa0, 0xa0)).toEqual({
						css: '#a0a0a0',
						rgba: 0xa0a0a0ff
					});
					expect(channels.toColor(0xb0, 0xb0, 0xb0)).toEqual({
						css: '#b0b0b0',
						rgba: 0xb0b0b0ff
					});
					expect(channels.toColor(0xc0, 0xc0, 0xc0)).toEqual({
						css: '#c0c0c0',
						rgba: 0xc0c0c0ff
					});
					expect(channels.toColor(0xd0, 0xd0, 0xd0)).toEqual({
						css: '#d0d0d0',
						rgba: 0xd0d0d0ff
					});
					expect(channels.toColor(0xe0, 0xe0, 0xe0)).toEqual({
						css: '#e0e0e0',
						rgba: 0xe0e0e0ff
					});
					expect(channels.toColor(0xf0, 0xf0, 0xf0)).toEqual({
						css: '#f0f0f0',
						rgba: 0xf0f0f0ff
					});
					expect(channels.toColor(0xff, 0xff, 0xff)).toEqual({
						css: '#ffffff',
						rgba: 0xffffffff
					});
				});
		...
		describe('css', () => {
			describe('toColor', () => {
				...
				it('should convert the rgb() format to an IColor', () => {
					...
					expect(css.toColor('rgb(255, 255, 255)')).toEqual({
						css: '#ffffffff',
						rgba: 0xffffffff
					});
				});
				it('should convert the rgba() format to an IColor', () => {
					expect(css.toColor('rgba(0, 0, 0, 0)')).toEqual({
						css: '#00000000',
						rgba: 0x00000000
					});
					expect(css.toColor('rgba(80, 0, 0, 0.5)')).toEqual({
						css: '#50000080',
						rgba: 0x50000080
					});
					expect(css.toColor('rgba(0, 80, 0, 0.5)')).toEqual({
						css: '#00500080',
						rgba: 0x00500080
					});
					expect(css.toColor('rgba(0, 0, 80, 0.5)')).toEqual({
						css: '#00005080',
						rgba: 0x00005080
					});
