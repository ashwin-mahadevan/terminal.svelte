...
describe('Keyboard', () => {
	describe('evaluateKeyEscapeSequence', () => {
		...
		it('should return \\x1b\\x08 for ctrl+alt+backspace', () => {
			expect(testEvaluateKeyboardEvent({ ctrlKey: true, altKey: true, keyCode: 8 }).key).toBe(
				'\x1b\x08'
			);
		});
		it('should return \\x1b[3;2~ for shift+delete', () => {
		...
		describe('On non-macOS platforms', () => {
			...
			it('should return \\x1b[1;3D for alt+left', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 37 }, { isMac: false }).key).toBe(
					'\x1b[1;3D'
				); // CSI 1;3 D
			});
			it('should return \\x1b[1;3C for alt+right', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 39 }, { isMac: false }).key).toBe(
					'\x1b[1;3C'
				); // CSI 1;3 C
			});
			it('should return \\x1b[1;3A for alt+up', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 38 }, { isMac: false }).key).toBe(
					'\x1b[1;3A'
				); // CSI 1;3 A
			});
			it('should return \\x1b[1;3B for alt+down', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 40 }, { isMac: false }).key).toBe(
					'\x1b[1;3B'
				); // CSI 1;3 B
			});
			it('should return \\x1ba for alt+a', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 65 }, { isMac: false }).key).toBe(
					'\x1ba'
				);
			});
			it('should return \\x1b\\x20 for alt+space', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 32 }, { isMac: false }).key).toBe(
					'\x1b\x20'
				);
			});
			it('should return \\x1b\\x00 for ctrl+alt+space', () => {
				expect(
					testEvaluateKeyboardEvent({ altKey: true, ctrlKey: true, keyCode: 32 }, { isMac: false })
						.key
				).toBe('\x1b\x00');
			});
		});
		...
		describe('On macOS platforms', () => {
			it('should return \\x1b[1;3D for alt+left', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 37 }, { isMac: true }).key).toBe(
					'\x1b[1;3D'
				); // CSI 1;3 D
			});
			it('should return \\x1b[1;3C for alt+right', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 39 }, { isMac: true }).key).toBe(
					'\x1b[1;3C'
				); // CSI 1;3 C
			});
			it('should return \\x1b[1;3A for alt+up', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 38 }, { isMac: true }).key).toBe(
					'\x1b[1;3A'
				); // CSI 1;3 A
			});
			it('should return \\x1b[1;3B for alt+down', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 40 }, { isMac: true }).key).toBe(
					'\x1b[1;3B'
				); // CSI 1;3 B
			});
			it('should return undefined for alt+a', () => {
				expect(testEvaluateKeyboardEvent({ altKey: true, keyCode: 65 }, { isMac: true }).key).toBe(
					undefined
				);
			});
		});
		...
		it('should return proper sequence for ctrl+alt+a', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, ctrlKey: true, keyCode: 65 }).key).toBe(
				'\x1b\x01'
			);
		});
		...
		it('should return proper sequences for alt+0', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 48 }).key).toBe(
				'\x1b0'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 48 }).key).toBe(
				'\x1b)'
			);
		});
		it('should return proper sequences for alt+1', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 49 }).key).toBe(
				'\x1b1'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 49 }).key).toBe(
				'\x1b!'
			);
		});
		it('should return proper sequences for alt+2', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 50 }).key).toBe(
				'\x1b2'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 50 }).key).toBe(
				'\x1b@'
			);
		});
		it('should return proper sequences for alt+3', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 51 }).key).toBe(
				'\x1b3'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 51 }).key).toBe(
				'\x1b#'
			);
		});
		it('should return proper sequences for alt+4', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 52 }).key).toBe(
				'\x1b4'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 52 }).key).toBe(
				'\x1b$'
			);
		});
		it('should return proper sequences for alt+5', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 53 }).key).toBe(
				'\x1b5'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 53 }).key).toBe(
				'\x1b%'
			);
		});
		it('should return proper sequences for alt+6', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 54 }).key).toBe(
				'\x1b6'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 54 }).key).toBe(
				'\x1b^'
			);
		});
		it('should return proper sequences for alt+7', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 55 }).key).toBe(
				'\x1b7'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 55 }).key).toBe(
				'\x1b&'
			);
		});
		it('should return proper sequences for alt+8', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 56 }).key).toBe(
				'\x1b8'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 56 }).key).toBe(
				'\x1b*'
			);
		});
		it('should return proper sequences for alt+9', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 57 }).key).toBe(
				'\x1b9'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 57 }).key).toBe(
				'\x1b('
			);
		});
		...
		it('should return proper sequences for alt+;', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 186 }).key).toBe(
				'\x1b;'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 186 }).key).toBe(
				'\x1b:'
			);
		});
		it('should return proper sequences for alt+=', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 187 }).key).toBe(
				'\x1b='
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 187 }).key).toBe(
				'\x1b+'
			);
		});
		it('should return proper sequences for alt+,', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 188 }).key).toBe(
				'\x1b,'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 188 }).key).toBe(
				'\x1b<'
			);
		});
		it('should return proper sequences for alt+-', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 189 }).key).toBe(
				'\x1b-'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 189 }).key).toBe(
				'\x1b_'
			);
		});
		it('should return proper sequences for alt+.', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 190 }).key).toBe(
				'\x1b.'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 190 }).key).toBe(
				'\x1b>'
			);
		});
		it('should return proper sequences for alt+/', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 191 }).key).toBe(
				'\x1b/'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 191 }).key).toBe(
				'\x1b?'
			);
		});
		it('should return proper sequences for alt+~', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 192 }).key).toBe(
				'\x1b`'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 192 }).key).toBe(
				'\x1b~'
			);
		});
		it('should return proper sequences for alt+[', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 219 }).key).toBe(
				'\x1b['
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 219 }).key).toBe(
				'\x1b{'
			);
		});
		it('should return proper sequences for alt+\\', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 220 }).key).toBe(
				'\x1b\\'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 220 }).key).toBe(
				'\x1b|'
			);
		});
		it('should return proper sequences for alt+]', () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 221 }).key).toBe(
				'\x1b]'
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 221 }).key).toBe(
				'\x1b}'
			);
		});
		it("should return proper sequences for alt+'", () => {
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 222 }).key).toBe(
				"\x1b'"
			);
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 222 }).key).toBe(
				'\x1b"'
			);
		});

		it('should handle mobile arrow events', () => {
			expect(testEvaluateKeyboardEvent({ keyCode: 0, key: 'UIKeyInputUpArrow' }).key).toBe(
				'\x1b[A'
			);
			expect(
				testEvaluateKeyboardEvent(
			...
			expect(testEvaluateKeyboardEvent({ keyCode: 0, key: 'UIKeyInputLeftArrow' }).key).toBe(
				'\x1b[D'
			);
			expect(
				testEvaluateKeyboardEvent(
			...
			expect(testEvaluateKeyboardEvent({ keyCode: 0, key: 'UIKeyInputRightArrow' }).key).toBe(
				'\x1b[C'
			);
			expect(
				testEvaluateKeyboardEvent(
			...
			expect(testEvaluateKeyboardEvent({ keyCode: 0, key: 'UIKeyInputDownArrow' }).key).toBe(
				'\x1b[B'
			);
			expect(
				testEvaluateKeyboardEvent(
		...
		it('should return proper sequences for alt+shift+letter combinations', () => {
			...
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 65 }).key).toBe(
				'\x1bA'
			); // alt+shift+a
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 72 }).key).toBe(
				'\x1bH'
			); // alt+shift+h
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: true, keyCode: 90 }).key).toBe(
				'\x1bZ'
			); // alt+shift+z

			// Test alt without shift produces lowercase letters
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 65 }).key).toBe(
				'\x1ba'
			); // alt+a
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 72 }).key).toBe(
				'\x1bh'
			); // alt+h
			expect(testEvaluateKeyboardEvent({ altKey: true, shiftKey: false, keyCode: 90 }).key).toBe(
				'\x1bz'
			); // alt+z
		});
