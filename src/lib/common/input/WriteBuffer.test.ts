...
describe('WriteBuffer', () => {
  ...
  describe('write input', () => {
    it('string', () => new Promise<void>(done => {
      wb.write('a._');
    ...
    it('bytes', () => new Promise<void>(done => {
      wb.write(toBytes('a._'));
      wb.write(toBytes('b.x'), () => { cbStack.push('b'); });
      wb.write(toBytes('c._'));
      wb.write(toBytes('d.x'), () => { cbStack.push('d'); });
      wb.write(toBytes('e'), () => {
        expect(stack.map(val => typeof val === 'string' ? '' :  fromBytes(val))).toEqual(['a._', 'b.x', 'c._', 'd.x', 'e']);
        expect(cbStack).toEqual(['b', 'd']);
        done();
      });
    }));
    it('string/bytes mixed', () => new Promise<void>(done => {
      wb.write('a._');
      wb.write('b.x', () => { cbStack.push('b'); });
      wb.write(toBytes('c._'));
      wb.write(toBytes('d.x'), () => { cbStack.push('d'); });
      wb.write(toBytes('e'), () => {
        expect(stack.map(val => typeof val === 'string' ? val :  fromBytes(val))).toEqual(['a._', 'b.x', 'c._', 'd.x', 'e']);
        expect(cbStack).toEqual(['b', 'd']);
        done();
      });
    }));
    it('write callback works for empty chunks', () => new Promise<void>(done => {
      ...
      wb.write('e', () => {
        expect(stack.map(val => typeof val === 'string' ? val :  fromBytes(val))).toEqual(['a', '', 'c', '', 'e']);
        expect(cbStack).toEqual(['a', 'b', 'c', 'd']);
        done();
      });
    }));
    it('writeSync', () => new Promise<void>(done => {
      wb.write('a', () => { cbStack.push('a'); });
    ...
    it('flushSync processes all pending writes', () => new Promise<void>(done => {
      wb.write('a', () => { cbStack.push('a'); });
      wb.write('b', () => { cbStack.push('b'); });
      wb.write('c', () => { cbStack.push('c'); });
      wb.flushSync();
    ...
    it('dispose cancels scheduled innerWrite', () => new Promise<void>(done => {
      wb.write('a');
      wb.dispose();
      setTimeout(() => {
        expect(stack).toEqual([]);
    ...
    it('dispose does not fire onWriteParsed for pending writes', () => new Promise<void>(done => {
      let parsed = 0;
      wb.onWriteParsed(() => parsed++);
      wb.write('a');
      wb.dispose();
    ...
    it('write after dispose is a no-op', () => new Promise<void>(done => {
      wb.dispose();
      wb.write('a');
      setTimeout(() => {
        expect(stack).toEqual([]);
    ...
    it('dispose is idempotent', () => new Promise<void>(done => {
      wb.write('a');
      wb.dispose();
      wb.dispose();
      setTimeout(() => {
    ...
    it('async handler continuation is skipped after dispose', () => new Promise<void>(done => {
    ...
    it('flushSync after dispose is a no-op', () => new Promise<void>(done => {
      wb.write('a');
      wb.dispose();
      wb.flushSync();
      setTimeout(() => {
