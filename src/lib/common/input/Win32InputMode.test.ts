...
describe('Win32InputMode', () => {
	describe('evaluateKeyboardEvent', () => {
		describe('basic key encoding', () => {
			...
			it('Enter key', () =>
				test(new Win32InputMode(), { code: 'Enter', key: 'Enter', keyCode: 13 }, true, (p) =>
			...
			it('Space key', () =>
				test(new Win32InputMode(), { code: 'Space', key: ' ', keyCode: 32 }, true, (p) =>
		...
		describe('navigation keys (ENHANCED_KEY)', () => {
			...
			navKeys.forEach(([code, key, keyCode, vk]) => {
				it(code, () =>
					test(new Win32InputMode(), { code, key, keyCode }, true, (p) => {
						expect(p!.vk).toBe(vk);
						expect(p!.cs & Win32ControlKeyState.ENHANCED_KEY).toBeTruthy();
					})
				);
			...
			it('Tab', () =>
				test(new Win32InputMode(), { code: 'Tab', key: 'Tab', keyCode: 9 }, true, (p) =>
					expect([p!.vk, p!.uc]).toEqual([0x09, 9])
				));
			it('Backspace', () =>
				test(new Win32InputMode(), { code: 'Backspace', key: 'Backspace', keyCode: 8 }, true, (p) =>
		...
		describe('numpad keys', () => {
			...
			it('NumpadMultiply', () =>
				test(new Win32InputMode(), { code: 'NumpadMultiply', key: '*', keyCode: 106 }, true, (p) =>
					expect(p!.vk).toBe(0x6a)
				));
			it('NumpadDivide (ENHANCED)', () =>
				test(new Win32InputMode(), { code: 'NumpadDivide', key: '/', keyCode: 111 }, true, (p) => {
					expect(p!.vk).toBe(0x6f);
					expect(p!.cs & Win32ControlKeyState.ENHANCED_KEY).toBeTruthy();
				}));
			it('NumpadDecimal', () =>
				test(new Win32InputMode(), { code: 'NumpadDecimal', key: '.', keyCode: 110 }, true, (p) =>
