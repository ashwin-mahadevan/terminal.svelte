...
import { IBufferLine, IAttributeData, IColorEvent, ColorRequestType, SpecialColorIndex } from '$lib/common/Types';
...
import { MockCoreService, MockBufferService, MockOptionsService, MockLogService, MockMouseStateService, MockCharsetService, MockUnicodeService, MockOscLinkService, extendedAttributes } from '$lib/common/TestUtils';
...
class TestInputHandler extends InputHandler {
  ...
  public get windowTitleStack(): string[] { return (this as any)._windowTitleStack; }
  public get iconNameStack(): string[] { return (this as any)._iconNameStack; }
...
describe('InputHandler', () => {
  ...
  describe('SL/SR/DECIC/DECDC', () => {
    ...
    it('SR (scrollRight)', async () => {
      ...
      expect(getLines(bufferService, 6)).toEqual(['12345', ' 1234', ' 1234', ' 1234', ' 1234', ' 1234']);
      await inputHandler.parseP('\x1b[0 A');
      expect(getLines(bufferService, 6)).toEqual(['12345', '  123', '  123', '  123', '  123', '  123']);
      await inputHandler.parseP('\x1b[2 A');
      expect(getLines(bufferService, 6)).toEqual(['12345', '    1', '    1', '    1', '    1', '    1']);
    });
    it('insertColumns (DECIC)', async () => {
      await inputHandler.parseP('12345'.repeat(6));
      await inputHandler.parseP('\x1b[3;3H');
      await inputHandler.parseP('\x1b[\'}');
      expect(getLines(bufferService, 6)).toEqual(['12345', '12 34', '12 34', '12 34', '12 34', '12 34']);
      bufferService.reset();
      await inputHandler.parseP('12345'.repeat(6));
      await inputHandler.parseP('\x1b[3;3H');
      ...
      await inputHandler.parseP('\x1b[3;3H');
      await inputHandler.parseP('\x1b[2\'}');
      expect(getLines(bufferService, 6)).toEqual(['12345', '12  3', '12  3', '12  3', '12  3', '12  3']);
    });
  ...
  describe('BS with reverseWraparound set/unset', () => {
    ...
    describe('reverseWraparound set', () => {
      it('should not reverse outside of scroll margins', async () => {
        ...
        expect(getLines(bufferService, 6)).toEqual(['#####', 'abcde', 'fghij', 'klmno', 'pqrst', 'uvwxy']);
        ...
        expect(getLines(bufferService, 6)).toEqual(['#####', 'abcde', 'fghij', 'klmno', 'pqrst', '    y']);
        ...
        await inputHandler.parseP(ttyBS.repeat(100));
        expect(getLines(bufferService, 6)).toEqual(['#####', 'abcde', 'fghij', 'klmno', 'pqrst', '     ']);
        ...
        await inputHandler.parseP(ttyBS.repeat(100));
        expect(getLines(bufferService, 6)).toEqual(['#####', 'abcde', '     ', '     ', '     ', 'uvwxy']);
        ...
        await inputHandler.parseP(ttyBS.repeat(100));
        expect(getLines(bufferService, 6)).toEqual(['#####', '     ', 'fghij', 'klmno', 'pqrst', 'uvwxy']);
      });
    });
  });
  ...
  describe('regression tests', () => {
    ...
    it('insertChars', async () => {
      ...
      expect(line1.translateToString(false)).toBe('a'.repeat(bufferService.cols - 10) + '1234567890');
      ...
      expect(line1.translateToString(false)).toBe('a'.repeat(bufferService.cols - 10) + ' 123456789');

      // insert one char from params = [1]
      bufferService.buffer.y = 0;
      bufferService.buffer.x = 70;
      inputHandler.insertChars(Params.fromArray([1]));
      expect(line1.translateToString(false)).toBe('a'.repeat(bufferService.cols - 10) + '  12345678');

      // insert two chars from params = [2]
      bufferService.buffer.y = 0;
      bufferService.buffer.x = 70;
      inputHandler.insertChars(Params.fromArray([2]));
      expect(line1.translateToString(false)).toBe('a'.repeat(bufferService.cols - 10) + '    123456');

      // insert 10 chars from params = [10]
      bufferService.buffer.y = 0;
      bufferService.buffer.x = 70;
      inputHandler.insertChars(Params.fromArray([10]));
      expect(line1.translateToString(false)).toBe('a'.repeat(bufferService.cols - 10) + '          ');
    ...
    it('deleteChars', async () => {
      ...
      expect(line1.translateToString(false)).toBe('a'.repeat(bufferService.cols - 10) + '234567890 ');
      ...
      expect(line1.translateToString(false)).toBe('a'.repeat(bufferService.cols - 10) + '34567890  ');
      ...
      expect(line1.translateToString(false)).toBe('a'.repeat(bufferService.cols - 10) + '567890    ');
    ...
    it('eraseInLine', async () => {
      ...
      expect(bufferService.buffer.lines.get(0)!.translateToString(false)).toBe('a'.repeat(70) + '          ');

      // params[1] - left erase
      bufferService.buffer.y = 1;
      bufferService.buffer.x = 70;
      inputHandler.eraseInLine(Params.fromArray([1]));
      expect(bufferService.buffer.lines.get(1)!.translateToString(false)).toBe(' '.repeat(70) + ' aaaaaaaaa');

      // params[1] - left erase
      bufferService.buffer.y = 2;
      bufferService.buffer.x = 70;
      inputHandler.eraseInLine(Params.fromArray([2]));
      expect(bufferService.buffer.lines.get(2)!.translateToString(false)).toBe(' '.repeat(bufferService.cols));
  ...
  describe('alt screen', () => {
    ...
    it('should handle DECSET/DECRST 1048 (alt screen cursor)', async () => {
      ...
      expect(bufferService.buffer.lines.get(0)!.loadCell(0, new CellData()).fg).toBe(DEFAULT_ATTR_DATA.fg);
  ...
  describe('underline colors (SGR 58 & SGR 59)', () => {
    ...
    it('defaults to FG color', async () => {
      for (const s of ['', '\x1b[30m', '\x1b[38;510m', '\x1b[38;2;1;2;3m']) {
        ...
        expect(inputHandler.curAttrData.getUnderlineColor()).toBe(inputHandler.curAttrData.getFgColor());
        expect(inputHandler.curAttrData.getUnderlineColorMode()).toBe(inputHandler.curAttrData.getFgColorMode());
        expect(inputHandler.curAttrData.isUnderlineColorRGB()).toBe(inputHandler.curAttrData.isFgRGB());
        expect(inputHandler.curAttrData.isUnderlineColorPalette()).toBe(inputHandler.curAttrData.isFgPalette());
        expect(inputHandler.curAttrData.isUnderlineColorDefault()).toBe(inputHandler.curAttrData.isFgDefault());
      }
    });
  ...
  describe('OSC', () => {
    it('4: query color events', async () => {
      ...
      expect(stack).toEqual([[{ type: ColorRequestType.REPORT, index: 0 }, { type: ColorRequestType.REPORT, index: 123 }]]);
      stack.length = 0;
    });
    it('4: set color events', async () => {
      ...
      expect(stack).toEqual([[{ type: ColorRequestType.SET, index: 0, color: [170, 187, 204] }, { type: ColorRequestType.SET, index: 123, color: [0, 17, 34] }]]);
      stack.length = 0;
    });
    ...
    it('104: restore events', async () => {
      ...
      expect(stack).toEqual([[{ type: ColorRequestType.RESTORE, index: 0 }], [{ type: ColorRequestType.RESTORE, index: 43 }]]);
      stack.length = 0;
    ...
    it('10: FG set & query events', async () => {
      ...
      expect(stack).toEqual([[{ type: ColorRequestType.REPORT, index: SpecialColorIndex.FOREGROUND }]]);
      stack.length = 0;
      // OSC with multiple values maps to OSC 10 & OSC 11 & OSC 12
      await inputHandler.parseP('\x1b]10;?;?;?;?\x07');
      expect(stack).toEqual([[{ type: ColorRequestType.REPORT, index: SpecialColorIndex.FOREGROUND }], [{ type: ColorRequestType.REPORT, index: SpecialColorIndex.BACKGROUND }], [{ type: ColorRequestType.REPORT, index: SpecialColorIndex.CURSOR }]]);
      stack.length = 0;
      // set foreground color events
      await inputHandler.parseP('\x1b]10;rgb:01/02/03\x07');
      expect(stack).toEqual([[{ type: ColorRequestType.SET, index: SpecialColorIndex.FOREGROUND, color: [1, 2, 3] }]]);
      stack.length = 0;
      await inputHandler.parseP('\x1b]10;#aabbcc\x07');
      expect(stack).toEqual([[{ type: ColorRequestType.SET, index: SpecialColorIndex.FOREGROUND, color: [170, 187, 204] }]]);
      stack.length = 0;
    ...
    it('110: restore FG color', async () => {
      ...
      expect(stack).toEqual([[{ type: ColorRequestType.RESTORE, index: SpecialColorIndex.FOREGROUND }]]);
    });
    it('11: BG set & query events', async () => {
      ...
      expect(stack).toEqual([[{ type: ColorRequestType.REPORT, index: SpecialColorIndex.BACKGROUND }]]);
      stack.length = 0;
      // OSC 11 with multiple values creates only BG and cursor event
      await inputHandler.parseP('\x1b]11;?;?;?;?\x07');
      expect(stack).toEqual([[{ type: ColorRequestType.REPORT, index: SpecialColorIndex.BACKGROUND }], [{ type: ColorRequestType.REPORT, index: SpecialColorIndex.CURSOR }]]);
      stack.length = 0;
      // set background color events
      await inputHandler.parseP('\x1b]11;rgb:01/02/03\x07');
      expect(stack).toEqual([[{ type: ColorRequestType.SET, index: SpecialColorIndex.BACKGROUND, color: [1, 2, 3] }]]);
      stack.length = 0;
      await inputHandler.parseP('\x1b]11;#aabbcc\x07');
      expect(stack).toEqual([[{ type: ColorRequestType.SET, index: SpecialColorIndex.BACKGROUND, color: [170, 187, 204] }]]);
      stack.length = 0;
    ...
    it('111: restore BG color', async () => {
      ...
      expect(stack).toEqual([[{ type: ColorRequestType.RESTORE, index: SpecialColorIndex.BACKGROUND }]]);
    });
    it('12: cursor color set & query events', async () => {
      ...
      expect(stack).toEqual([[{ type: ColorRequestType.SET, index: SpecialColorIndex.CURSOR, color: [1, 2, 3] }]]);
      stack.length = 0;
      await inputHandler.parseP('\x1b]12;#aabbcc\x07');
      expect(stack).toEqual([[{ type: ColorRequestType.SET, index: SpecialColorIndex.CURSOR, color: [170, 187, 204] }]]);
    });
    it('112: restore cursor color', async () => {
      const stack: IColorEvent[] = [];
      inputHandler.onColor(ev => stack.push(ev));
      await inputHandler.parseP('\x1b]112\x07');
      expect(stack).toEqual([[{ type: ColorRequestType.RESTORE, index: SpecialColorIndex.CURSOR }]]);
    });
  });
