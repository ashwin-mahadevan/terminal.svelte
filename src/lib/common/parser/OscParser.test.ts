...
describe('OscParser', () => {
  ...
  describe('identifier parsing', () => {
    ...
    it('no payload', () => {
      ...
      parser.put(data, 0, data.length);
      parser.end(true);
      expect(reports).toEqual([[1234, 'START', undefined], [1234, 'END', true]]);
    });
    it('with payload', () => {
      parser.start();
      let data = toUtf32('12');
  ...
  describe('OscHandlerFactory', () => {
    ...
    it('should be disposable', () => {
      ...
      parser.put(data, 0, data.length);
      parser.end(true);
      expect(reports).toEqual([['two', 'Here comes the mouse!'], ['one', 'some other data']]);
    });
    it('should respect return false', () => {
      ...
      parser.put(data, 0, data.length);
      parser.end(true);
      expect(reports).toEqual([['two', 'Here comes the mouse!'], ['one', 'Here comes the mouse!']]);
    });
