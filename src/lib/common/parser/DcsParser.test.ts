...
describe('DcsParser', () => {
  ...
  describe('DcsHandlerFactory', () => {
    ...
    it('should be disposable', () => {
      ...
      parser.put(data, 0, data.length);
      parser.unhook(true);
      expect(reports).toEqual([['two', [1, 2, 3], 'Here comes the mouse!'], ['one', [1, 2, 3], 'some other data']]);
    });
    it('should respect return false', () => {
      ...
      parser.put(data, 0, data.length);
      parser.unhook(true);
      expect(reports).toEqual([['two', [1, 2, 3], 'Here comes the mouse!'], ['one', [1, 2, 3], 'Here comes the mouse!']]);
    });
