...
} from '$lib/common/parser/EscapeSequenceParser';
import { StringToUtf32, stringFromCodePoint, utf32ToString } from '$lib/common/input/TextDecoder';
		...
		describe('registration limits', () => {
			it('prefix range 0x3c .. 0x3f, one byte', () => {
				for (let i = 0x3c; i <= 0x3f; ++i) {
					...
					expect(parser.identToString(parser.identifier({ prefix: c, final: 'z' }))).toBe(c + 'z');
				}
				expect(() => {
