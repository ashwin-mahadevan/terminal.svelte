...
import { IParsingState, IParams, ParamsArray, IOscParser, IOscHandler, OscFallbackHandlerType, IFunctionIdentifier, IParserStackState, ParserStackType } from '$lib/common/parser/Types';
import { EscapeSequenceParser, TransitionTable, VT500_TRANSITION_TABLE } from '$lib/common/parser/EscapeSequenceParser';
  ...
  describe('set/clear handler', () => {
    ...
    describe('CSI custom handlers', () => {
      it('Prevent fallback', () => {
        ...
        expect(csiCustom).toEqual([['m', [1, 31], ''], ['m', [0], '']]);
      });
      it('Allow fallback', () => {
        const csiCustom: [string, ParamsArray, string][] = [];
        parser2.registerCsiHandler({ final: 'm' }, params => { csi.push(['m', params.toArray(), '']); return true; });
        ...
        parse(parser2, INPUT);
        expect(csi).toEqual([['m', [1, 31], ''], ['m', [0], '']]); // Should fallback to original handler
        expect(csiCustom).toEqual([['m', [1, 31], ''], ['m', [0], '']]);
      });
      it('Multiple custom handlers fallback once', () => {
        const csiCustom: [string, ParamsArray, string][] = [];
        ...
        expect(csi).toEqual([]); // Should not fallback to original handler
        expect(csiCustom).toEqual([['m', [1, 31], ''], ['m', [0], '']]);
        expect(csiCustom2).toEqual([['m', [1, 31], ''], ['m', [0], '']]);
      });
      it('Multiple custom handlers no fallback', () => {
        const csiCustom: [string, ParamsArray, string][] = [];
        const csiCustom2: [string, ParamsArray, string][] = [];
    ...
    describe('DCS custom handlers', () => {
      ...
      it('Allow fallback', () => {
        ...
        parse(parser2, DCS_INPUT);
        expect(dcsCustom).toEqual([['B', [1, 2, 3], 'abc'], ['A', [1, 2, 3], 'abc']]);
      });
      it('Multiple custom handlers fallback once', () => {
        const dcsCustom: [string, (number | number[])[], string][] = [];
        parser2.registerDcsHandler({ intermediates: '+', final: 'p' }, new DcsHandler((data, params) => { dcsCustom.push(['A', params.toArray(), data]); return true; }));
        ...
        parse(parser2, DCS_INPUT);
        expect(dcsCustom).toEqual([['C', [1, 2, 3], 'abc'], ['B', [1, 2, 3], 'abc']]);
      });
      it('Multiple custom handlers no fallback', () => {
        const dcsCustom: [string, (number | number[])[], string][] = [];
        parser2.registerDcsHandler({ intermediates: '+', final: 'p' }, new DcsHandler((data, params) => { dcsCustom.push(['A', params.toArray(), data]); return true; }));
    ...
    describe('APC custom handlers', () => {
      ...
      it('Allow fallback', () => {
        ...
        parse(parser2, APC_INPUT);
        expect(apcCustom).toEqual([['B', 'abc'], ['A', 'abc']]);
      });
      it('Multiple custom handlers fallback once', () => {
        const apcCustom: [string, string][] = [];
        parser2.registerApcHandler({ intermediates: '+', final: 'p' }, new ApcHandler(data => { apcCustom.push(['A', data]); return true; }));
        ...
        parse(parser2, APC_INPUT);
        expect(apcCustom).toEqual([['C', 'abc'], ['B', 'abc']]);
      });
      it('Multiple custom handlers no fallback', () => {
        const apcCustom: [string, string][] = [];
        parser2.registerApcHandler({ intermediates: '+', final: 'p' }, new ApcHandler(data => { apcCustom.push(['A', data]); return true; }));
  ...
  describe('function identifiers', () => {
    describe('registration limits', () => {
      it('prefix range 0x3c .. 0x3f, one byte', () => {
        ...
        expect(() => { parser.identifier({ prefix: '\x3b', final: 'z' }); }).toThrow('prefix must be in range 0x3c .. 0x3f');
        expect(() => { parser.identifier({ prefix: '\x40', final: 'z' }); }).toThrow('prefix must be in range 0x3c .. 0x3f');
        expect(() => { parser.identifier({ prefix: '??', final: 'z' }); }).toThrow('only one byte as prefix supported');
      });
      it('intermediates range 0x20 .. 0x2f, up to two bytes', () => {
        for (let i = 0x20; i <= 0x2f; ++i) {
          const c = String.fromCharCode(i);
          expect(parser.identToString(parser.identifier({ intermediates: c + c, final: 'z' }))).toBe(c + c + 'z');
        }
        expect(() => { parser.identifier({ intermediates: '\x1f', final: 'z' }); }).toThrow('intermediate must be in range 0x20 .. 0x2f');
        expect(() => { parser.identifier({ intermediates: '\x30', final: 'z' }); }).toThrow('intermediate must be in range 0x20 .. 0x2f');
        expect(() => { parser.identifier({ intermediates: '!!!', final: 'z' }); }).toThrow('only two bytes as intermediates are supported');
      });
      it('final CSI/DCS range 0x40 .. 0x7e (default), one byte', () => {
        ...
        expect(() => { parser.identifier({ final: '\x3f' }); }).toThrow('final must be in range 64 .. 126');
        expect(() => { parser.identifier({ final: '\x7f' }); }).toThrow('final must be in range 64 .. 126');
        expect(() => { parser.identifier({ final: 'zz' }); }).toThrow('final must be a single byte');
      });
      it('final ESC + APC range 0x30 .. 0x7e, one byte', () => {
        for (let i = 0x30; i <= 0x7e; ++i) {
          ...
          expect(() => { handler = parser.registerEscHandler({ final }, () => true); }).not.toThrow();
        ...
        expect(() => { parser.registerEscHandler({ final: '\x2f' }, () => true); }).toThrow('final must be in range 48 .. 126');
        expect(() => { parser.registerEscHandler({ final: '\x7f' }, () => true); }).toThrow('final must be in range 48 .. 126');
        expect(() => { parser.registerApcHandler({ final: '\x2f' }, {start: () => {}, put: ()=>{}, end: ()=>true}); }).toThrow('final must be in range 48 .. 126');
        expect(() => { parser.registerApcHandler({ final: '\x7f' }, {start: () => {}, put: ()=>{}, end: ()=>true}); }).toThrow('final must be in range 48 .. 126');
      });
      it('id calculation - should stacking prefix -> intermediate -> final', () => {
        ...
        expect(parser.identToString(parser.identifier({ intermediates: '!', final: 'z' }))).toBe('!z');
        expect(parser.identToString(parser.identifier({ prefix: '?', intermediates: '!', final: 'z' }))).toBe('?!z');
        expect(parser.identToString(parser.identifier({ prefix: '?', intermediates: '!!', final: 'z' }))).toBe('?!!z');
      });
    });
...
describe('EscapeSequenceParser - async', () => {
  ...
  describe('async handlers', () => {
    ...
    it('improper continuation should throw', async () => {
      ...
      expect(!parseSync(parser, INPUT)).not.toBe(true);
      expect(callstack).not.toEqual(RESULT);
      expect(() => parseSync(parser, INPUT)).toThrow('improper continuation due to previous async handler, giving up parsing');
      // keeps being broken for further parse calls (sync and async)
      expect(() => parseSync(parser, 'random')).toThrow('improper continuation due to previous async handler, giving up parsing');
