...
import { MockOptionsService, MockBufferService, MockLogService, createCellData } from '$lib/common/TestUtils';
    ...
    it('should fill the buffer with blank lines based on the size of the viewport', () => {
      ...
      for (let y = 0; y < INIT_ROWS; y++) {
        ...
        for (let x = 0; x < INIT_COLS; x++) {
          expect(buffer.lines.get(y)!.loadCell(x, new CellData()).getAsCharData()).toEqual(blankLineChar);
        }
      }
    });
  });
  ...
  describe('getWrappedRangeForLine', () => {
    describe('non-wrapped', () => {
      ...
      it('should return a single row for the last row', () => {
        buffer.fillViewportRows();
        expect(buffer.getWrappedRangeForLine(buffer.lines.length - 1)).toEqual({ first: 23, last: 23 });
      });
    });
    describe('wrapped', () => {
      ...
      it('should return a range for the last row', () => {
        ...
        expect(buffer.getWrappedRangeForLine(buffer.lines.length - 1)).toEqual({ first: 22, last: 23 });
      });
      ...
      it('should return a range for a row that wraps downward to last row', () => {
        ...
        expect(buffer.getWrappedRangeForLine(buffer.lines.length - 2)).toEqual({ first: 22, last: 23 });
      });
    });
  });
  ...
  describe('resize', () => {
    ...
    describe('row size reduced', () => {
      ...
      describe('no scrollback', () => {
        it('should trim from the top of the buffer when the cursor reaches the bottom', () => {
          ...
          expect(buffer.lines.get(INIT_ROWS - 1 - 5)!.loadCell(0, new CellData()).getAsCharData()[1]).toBe('b');
        });
      });
    });
    ...
    describe('reflow', () => {
      ...
      it('should adjust markers when reflowing', () => {
        ...
        expect(secondMarker.line, 'second marker should be shifted since the first line wrapped').toBe(5);
        expect(thirdMarker.line, 'third marker should be shifted since the first and second lines wrapped').toBe(10);
        buffer.resize(10, 16);
        expect(buffer.lines.get(0)!.translateToString()).toBe('abcdefghij');
        expect(buffer.lines.get(1)!.translateToString()).toBe('0123456789');
        expect(buffer.lines.get(2)!.translateToString()).toBe('klmnopqrst');
      ...
      it('should dispose markers whose rows are trimmed during a reflow', () => {
        ...
        expect(secondMarker.line, 'second marker should remain the same as it was shifted 4 and trimmed 4').toBe(1);
        expect(thirdMarker.line, 'third marker should be shifted since the first and second lines wrapped').toBe(6);
        ...
        describe('viewport not yet filled', () => {
          it('should move the cursor up and add empty lines', () => {
            ...
            for (let i = 0; i < buffer.lines.length; i++) {
              expect(buffer.lines.get(i)!.isWrapped, `line ${i} isWrapped must equal ${wrappedLines.includes(i)}`).toBe(wrappedLines.includes(i));
            }
          });
        });
