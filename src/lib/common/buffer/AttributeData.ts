...
export class AttributeData implements IAttributeData {
	...
	public isFgPalette(): boolean {
		return (
			(this.fg & Attributes.CM_MASK) === Attributes.CM_P16 ||
			(this.fg & Attributes.CM_MASK) === Attributes.CM_P256
		);
	}
	public isBgPalette(): boolean {
		return (
			(this.bg & Attributes.CM_MASK) === Attributes.CM_P16 ||
			(this.bg & Attributes.CM_MASK) === Attributes.CM_P256
		);
	}
	...
	public getFgColor(): number {
		switch (this.fg & Attributes.CM_MASK) {
			...
			case Attributes.CM_P256:
				return this.fg & Attributes.PCOLOR_MASK;
			case Attributes.CM_RGB:
				return this.fg & Attributes.RGB_MASK;
			default:
	...
	public getBgColor(): number {
		switch (this.bg & Attributes.CM_MASK) {
			case Attributes.CM_P16:
			case Attributes.CM_P256:
				return this.bg & Attributes.PCOLOR_MASK;
			case Attributes.CM_RGB:
				return this.bg & Attributes.RGB_MASK;
			default:
				return -1; // CM_DEFAULT defaults to -1
		}
	}
...
export class ExtendedAttrs implements IExtendedAttrs {
	...
	public get ext(): number {
		if (this._urlId) {
			return (this._ext & ~ExtFlags.UNDERLINE_STYLE) | (this.underlineStyle << 26);
		}
		return this._ext;
	}
	...
	constructor(ext: number = 0, urlId: number = 0) {
		this._ext = ext;
		this._urlId = urlId;
	}
