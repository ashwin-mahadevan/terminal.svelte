...
export function infoToWidth(info: number, ambiguousIsWide = false): 0 | 1 | 2 {
	...
	return v < CHARWIDTH_EA_AMBIGUOUS ? 1 : v >= CHARWIDTH_WIDE || ambiguousIsWide ? 2 : 1;
}
...
export function columnToIndexInContext(
	str: string,
	startIndex: number,
	column: number,
	preferWide: boolean
...
export function shouldJoin(beforeState: number, afterInfo: number): number {
	// TODO: Fix this upstream lint error.
	...
	if (_shouldJoin(beforeCode, afterCode)) {
		if (afterCode === GRAPHEME_BREAK_Regional_Indicator) return GRAPHEME_BREAK_SAW_Regional_Pair;
		else return afterCode + 16;
	} else return afterCode - 16;
}
...
export function shouldJoinBackwards(beforeInfo: number, afterState: number): number {
	...
	if (_shouldJoin(beforeCode, afterCode)) {
		if (beforeCode === GRAPHEME_BREAK_Regional_Indicator) return GRAPHEME_BREAK_SAW_Regional_Pair;
		else return beforeCode + 16;
	} else return beforeCode - 16;
}

/** Doesn't handle an odd number of RI characters. */
function _shouldJoin(beforeCode: number, afterCode: number): boolean {
	if (beforeCode >= GRAPHEME_BREAK_Hangul_L && beforeCode <= GRAPHEME_BREAK_Hangul_LVT) {
		if (
			beforeCode == GRAPHEME_BREAK_Hangul_L && // GB6
			...
			return true;
		if (
			(beforeCode == GRAPHEME_BREAK_Hangul_LV || // GB7
				beforeCode == GRAPHEME_BREAK_Hangul_V) &&
			(afterCode == GRAPHEME_BREAK_Hangul_V || afterCode == GRAPHEME_BREAK_Hangul_T)
		)
			return true;
		if (
			(beforeCode == GRAPHEME_BREAK_Hangul_LVT || // GB8
	...
	if (
		afterCode == GRAPHEME_BREAK_Extend || // GB9
		...
		return true;
	if (
		beforeCode == GRAPHEME_BREAK_ZWJ && // GB11
		afterCode == GRAPHEME_BREAK_ExtPic
	)
		return true;
	if (
		afterCode == GRAPHEME_BREAK_Regional_Indicator && // GB12, GB13
